...
[ ] **ADR 0009 npc enrichment → ciclo implementativo** su `adr/0009-npc-enrichment` (S0 design pass 2026-07-07/08, ADR Proposed, advisor review Opus foldata — 11 finding, 1 reversal H2): S1 core (rekey UUID rung v7 ATOMICO con conversione di tutti i call-site name-keyed + modello engine tipizzato + taxonomy `npc_fields` + resolver F2 + scaffold D1/D2 + test B2), S2 tools (update_npc + fuzzy guard B4, kill/remove/restore, writer morte HP≤0, config), S3 surfaces (scene/npc prompt, editor npc_fields + prune-on-removal G5 incl. fix bug assi 0005, FE). Piano operativo in ADR §10.
