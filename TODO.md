...
[~] **UI redesign → ADR 0013** (Proposed, `docs/adr/0013-ui-visual-redesign.md`): dark moderno pulito **Voyage-informed, identità nostra**; dark-only; voce piana EN-first (tieni i18n, IT dopo); re-skin sull'architettura tokenizzata (NO rewrite); accento **cool arcane**; atmosfera stripped; **dado bespoke** (2-3 varianti in build, "tier arc" candidato); wordmark pulito. Font `@fontsource-variable/newsreader`+`instrument-sans`. Fondazione fatta (`750a2ac`: token dark + font sistema, branch `redesign/ui-dark-a`). PROSSIMO: **vertical slice play screen** verificata a screenshot headless → ok owner → propago alle altre schermate. Input della ricerca in `scratch/research/ui-redesign.md`, mappe stato-attuale + bug in `scratch/ui-map/`. TODO in-build: hex accento, variante dado. PARCHEGGIATO (ADR suoi): "player input come ispirazione" (gameplay), bug `getTurns` empty-render.
