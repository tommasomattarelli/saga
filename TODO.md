...
[~] **harness modelli (`backend/eval/`)**: fatto — 5 probe, scenari `empty`/`gold`/`bloat`/`contradiction`, colonna Δ, `--dry-run` per validare i prompt senza consumare quota. Il trascritto gold e' 25 turni ancorati al world reale, con lunghezza calibrata su produzione (1375 char/turno, misurati dal DB dev) e stato derivato replayando gli update negli handler veri. RESTA:
    [ ] **passo `--derive`** (COSTA QUOTA): far girare summarizer + fact extractor + global summary sui 25 turni per riempire `context.*` e `fact_corpus`. Finche' non gira, `gold` e `bloat` hanno i riassunti VUOTI — sono storia grezza + stato, non contesto saturo vero, e il harness lo avvisa a ogni run. Il Δ misurato oggi e' quindi una sottostima.
    [ ] **probe canarino di memoria**: pianta un fatto nei `recalled_memories` che il modello non puo' inventare e verifica che lo usi. Gateato dal `--derive`. NB: il harness inietta il recall direttamente, quindi si puo' testare anche con #55 aperto.
...
[ ] **audit `max_tokens` di tutte le chiamate LLM** (std 14): `dm_nodes.py:122` e' l'unico che passa `model_cfg.max_tokens`; gli altri quattro hardcodano e buttano il valore appena risolto dal router — `npc_director.py:79` (300 contro `npc_behavior: 1000`), `compressor.py:91` (300), `fact_extractor.py:87` (500), `global_summary.py:73` (600), questi tre su `MEMORY_COMPRESSION` che dichiara 500. Due problemi: gli hardcode, e la granularita' — un knob solo per tre job con output di taglia diversa. NB: sui modelli di reasoning i token di pensiero contano nel budget, quindi un cap basso restituisce JSON troncato — probabile causa del `dialogue: "..."` di `npc_director.py:99`.
