...
[ ] **le fixture scritte a mano certificano bachi** (trovato 2026-08-04 con l'audit chiavi). Il pattern: il test si costruisce lo stato di input E asserisce sulla resa in output, quindi la fixture e' una **seconda definizione dello schema** che nessuno valida contro il produttore. Quando divergono, il test pinna la forma sbagliata. `test_dm_prompt_xml.py` scriveva `{"hp": 12, "max_hp": 20}` e asseriva `hp="12/20"`: verde, mentre la produzione (`{current, max}`) rendeva un repr di dict. Stessa malattia dell'alembic qui sotto — il test fabbrica l'artefatto invece di esercitare il produttore. Uscite, in ordine di forza: (a) **tipizzare** — `NpcEngineRecord` e' Pydantic e non ha una chiave morta, i dict liberi le hanno tutte, e con un modello la fixture bugiarda non si scrive proprio (owner: 0010-F1); (b) **derivare le fixture dal seed vero** con una factory in `conftest.py`, cosi' non possono mentire per costruzione; (c) un test di conformita' fixture↔seed, dieci righe, prende la classe intera. NB: gli standard 1 e 5 lo dicevano gia' — non serve una filosofia nuova, serve applicarla.
