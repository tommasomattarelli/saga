...
[ ] installer PORTABILITÀ pacchetti Postgres (VALIDATO via podman, giu 2026). `install_saga.sh` fa `apt-get install postgresql-16 postgresql-16-pgvector` affidandosi ai repo default della distro → funziona SOLO su **Ubuntu 24.04** (noble ha pg16+pgvector in universe). **ROTTO su Debian 12** (ship pg15) e **Debian 13** (ship pg17): `Unable to locate package postgresql-16`. **Fedora NON supportato** (nessun ramo dnf; l'installer conosce solo apt/brew) — eppure pg16 + pgvector 0.6.2 sono in dnf. FIX robusto Debian/Ubuntu: aggiungere il repo **PGDG** (`apt.postgresql.org`) come già fa `installer-smoke.yml:71-75` a mano → fornisce pg16 uniformemente su tutte le release. Fedora: ramo dnf o documentare unsupported. TRADEOFF: scrivere una apt-source di sistema è più invasivo sulla macchina utente → decidere lo scope PRIMA di implementare.
