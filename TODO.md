...
[ ] **ADR 0008 world model → implementazione** (ciclo attivo, branch `adr/0008-world-model`, piano in ADR §9). Interview 2026-07-06: risolti J3 (NPC interim = node UUID), B4 (mappa read-only play screen, pergamena+pin v1, `map_image` riservato), I5b (git identity repo-local), vincolo accessor unico su C-iv. **Sprint 0 design pass FATTO (2026-07-06)**: chiusi TUTTI i TODO aperti — revisione headline **P0 vocabolari world-defined** (kind/terrain/mode custom per mondo, meta-schema al posto della discriminated union), layout dir-as-tree (filename=slug, no parent_id), game home `~/.saga`, colonne `world_baseline`+`world_state`, git in Docker image. PROSSIMO: S1 modelli+loader → S2 instantiation+persistence → S3 travel+runtime → S4 mappa FE → S5 editor. Sub-branch per sprint, merge su adr/0008, PR unica su main a fine ciclo.
