...
[ ] **ADR 0005 npc psychology → ciclo implementazione** (design fissato, S0 2026-07-07): assi world-defined in `taxonomy.yaml` (pattern P0 0008, default bundled trust/respect/affection/fear), bande nominate per-asse, `axis_changes` + cap + `met_player` ×3, tool `change_npc_psychology`, scena DM solo assi salienti, rung v5→v6 triviale (no lift, volume wipato). Branch `adr/0005-npc-psychology`, sub-branch `0005/s1-…`: S1 taxonomy+core (BE puro) → S2 superfici LLM → S3 editor (FE). PR unica a fine ciclo. Dettagli in `docs/adr/0005-npc-multi-axis-psychology.md` §9.
