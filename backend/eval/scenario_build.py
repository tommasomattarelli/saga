...
def assemble(meta: dict, turns: list[dict], build_cfg: dict, variant: str | None) -> dict:
    ...
    return {
        ...
        # Empty until eval/derive.py runs the real summariser / fact extractor.
        "context": {"global_summary": None, "summary_context": None, "recall": {}},
        "fact_corpus": [],
        "canaries": meta.get("canaries", {}),
        # Per-probe stimuli, authored with the transcript so they fit the scene it
        # ends in. The checks live in eval/probes.py and never move. `recall` is
        # dropped here on purpose — it is authored alongside the stimulus but its
        # resolved home is context.recall, written by derive.py.
        "probes": {
            name: {key: value for key, value in fields.items() if key != "recall"}
            for name, fields in meta.get("probes", {}).items()
        },
        "campaign": {
            "death_mode": "cronista",
...
def main() -> None:
    ...
    out.write_text(
        ...
        "# context/* and fact_corpus stay empty until eval/derive.py runs.\n"
