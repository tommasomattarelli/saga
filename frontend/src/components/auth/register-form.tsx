...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: tokens } = await register(username, email, password);
      setTokens(tokens);
    ...
    <div className="flex min-h-screen items-center justify-center">
      <div className="w-full max-w-md rounded-lg border border-parchment-700/30 bg-parchment-900/80 p-8 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm text-parchment-300">Email</label>
            <input
              type="email"
              value={email}
          ...
          >
            {loading ? "Forging your destiny..." : "Create Character"}
          </button>
        </form>
