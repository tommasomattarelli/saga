...
- **Release notes**: `release.sh` regroups `[Unreleased]`'s `### Highlights` **by area** — each bullet's `[Area]` tag becomes its `### <Area>` header (areas: Gameplay · World & DM · UI · Installer · Memory & AI · Infra · Docs). Append `(@handle)` for outside contributors and `(#NN)` when the release closes an issue; `### Internal` is not published.
